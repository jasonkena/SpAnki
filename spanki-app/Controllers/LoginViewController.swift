...
class LoginViewController: UIViewController {
    ...
    func signIn() {
        ...
        if authUI.auth?.currentUser == nil { // user has not signed in
            ...
            let spankiUser = spankiUser(user: currentUser)
            spankiUser.saveIfNewUser { success in
                if success {
                    self.performSegue(withIdentifier: "FirstShowSegue", sender: nil)
                } else {
                    print("ERROR: Tried to save a SpankiUser but failed.")
                }
            }
        }
    }
...
extension LoginViewController: FUIAuthDelegate {
    func authPickerViewController(forAuthUI authUI: FUIAuth) -> FUIAuthPickerViewController {
        ...
        logoImageView.image = UIImage(named: "checkmark.bubble")
